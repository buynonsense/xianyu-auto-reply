...
注意：结合商品信息，给出实用建议。'''
    ...
    def _call_openai_api(self, client: OpenAI, settings: dict, messages: list, max_tokens: int = 100, temperature: float = 0.7) -> str:
        ...
        保留/恢复：对于 GLM-4.5 系列（例如 glm-4.5-flash），我们会尝试通过 extra_body 关闭思维链（thinking）。
        仅在确实需要时注入额外参数，尽量不影响其他模型调用。
        """
        ...
        extra_body = None
        # 明确检测 glm-4.5-flash（同时包含 glm-4.5 的通用匹配）
        if 'glm-4.5-flash' in model_lower or 'glm-4.5' in model_lower:
            logger.info(f"检测到 GLM-4.5 系列模型 ({model})，尝试通过 extra_body 关闭思维链")
            extra_body = {"thinking": {"type": "disabled"}}

        # 构建调用参数，按需注入 extra_body
        call_kwargs = {
